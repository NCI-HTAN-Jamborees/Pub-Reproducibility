...
### download survival metadata from https://github.com/dpeerlab/SCLC_atlas-HTAN/tree/main/out.SCLC.060121/survival, which contains identical values to Table S16.
df_meta <- read.table("survival_metadata.PLCG2-high_scseq.tableS18.txt",
                      header=T,sep="\t")

df_meta$group <- "N"
df_meta$group[df_meta$Recurrent.Cluster...0.75..=="YES"] <- "Y"
df_meta$Time <- df_meta$Overall.Survival..months.
df_meta$Status <- 0
df_meta$Status[df_meta$Vital.Status == "Deceased"] <- 1

df_meta_1 <- df_meta[df_meta$Ever.ES.SCLC.=="YES",]

fit <- survfit(Surv(Time, Status) ~ group, data = df_meta_1)
...
g_to_save <- ggsave_workaround(plot1)

# Save the plot
ggsave("Kaplan-Meier_analysis_of_OS_of_PLCG2_Recurrent_Cluster_group.pdf",
       plot=g_to_save,height = 5.3,width = 3.7)


### correlation analysis
df_meta_1$Status = as.character(df_meta_1$Status)

plot2 <- ggplot(df_meta_1, aes(x=Time, y=Recurrent.Cluster....of.tumor.cells.)) + 
  geom_smooth(method=lm , color="red", se=T)+
  stat_cor(method = "spearman", label.x = 10, label.y = 0.9)+
  geom_point(size=6,aes(color=group, shape=Status)) +
  theme_bw()+theme(panel.grid =element_blank())+
  scale_y_continuous(trans='log10')+
  guides(color = guide_legend(order=1,title = "Recurrent Cluster"),
         shape = guide_legend(order=2,title = "Deceased"))+
  labs( y ="Percentage of recurrent cluster in each tumor")+theme(aspect.ratio=1)


ggsave("Scatterplot_Percent_of_recurrent_cluster_versus_overall_survival.pdf",
       plot=plot2,height = 4,width = 5)
